...
ranking_SR_Correlation_oos = rankings_oos['SR_Correlation']
...
serie_SR_Correlation_oos = series_oos['SR_Correlation']
...
ids_rolling_SR_Correlation = (ranking_SR_Correlation_oos.iloc[t_anterior].values - 1).astype(int)
...
R_rolling_SR_Correlation = R_oos[t_oos, ids_rolling_SR_Correlation]
...
R_cartera_SR_Correlation = pd.Series(R_rolling_SR_Correlation @ w, index=T)
...
wealth_rolling_SR_Correlation = W0 * (1 + R_cartera_SR_Correlation).cumprod()
...
ids_buyhold_SR_Correlation = ranking_SR_Correlation_oos.loc[t0].values
...
R_buyhold_SR_Correlation = df_rendimientos.loc[T, ids_buyhold_SR_Correlation]
...
etf_wealth_buyhold_SR_Correlation = W0 * (1 + R_buyhold_SR_Correlation).cumprod()
...
wealth_buyhold_SR_Correlation = pd.Series(etf_wealth_buyhold_SR_Correlation.values @ w, index=T)
...
replacement_rate_SR_Correlation, freq_SR_Correlation = replacement_rate(ranking_SR_Correlation_oos, T, return_frequency=True)
...
df_turnover_activos = pd.DataFrame({
    ...
    'Nuevos_SR_Correlation': replacement_rate_SR_Correlation,
...
print(f"   • Sharpe Correlation  : {df_turnover_activos['Nuevos_SR_Correlation'].mean():.4f} de {K} ETFs por día")
...
df_turnover_freq = pd.DataFrame({
    'SR': freq_SR,
    'SR_Correlation': freq_SR_Correlation,
    'MSR': freq_MSR,
    'VaRR1': freq_VaRR1,
    'VaRR5': freq_VaRR5,
    'VaRR10': freq_VaRR10,
...
print("\n   ► Sharpe Correlation :")
print(f"      • 0 ETFs nuevos: {df_turnover_freq['SR_Correlation'].get(0, 0)} veces")
print(f"      • 1 ETFs nuevos: {df_turnover_freq['SR_Correlation'].get(1, 0)} veces")
print(f"      • 2 ETFs nuevos: {df_turnover_freq['SR_Correlation'].get(2, 0)} veces")
print(f"      • 3 ETFs nuevos: {df_turnover_freq['SR_Correlation'].get(3, 0)} veces")
print(f"      • 4 ETFs nuevos: {df_turnover_freq['SR_Correlation'].get(4, 0)} veces")
...
plot_turnover_frequency({
        'SR': freq_SR,
        'Sharpe Correlation': freq_SR_Correlation,
        'MSR': freq_MSR,
        'VaRR1': freq_VaRR1,
        'VaRR5': freq_VaRR5,
        'VaRR10': freq_VaRR10,
...
stats_SR_Correlation = etf_stats(ids_rolling_SR_Correlation, tickers)
...
stats_SR_Correlation = stats_SR_Correlation.sort_values(by='Days_Active', ascending=False).reset_index(drop=True)
...
print("\n   ► Sharpe Correlation:")
print(f"      1. {stats_SR_Correlation.iloc[0]['Ticker']:<10} ({stats_SR_Correlation.iloc[0]['Days_Active']} días)")
print(f"      2. {stats_SR_Correlation.iloc[1]['Ticker']:<10} ({stats_SR_Correlation.iloc[1]['Days_Active']} días)")
print(f"      3. {stats_SR_Correlation.iloc[2]['Ticker']:<10} ({stats_SR_Correlation.iloc[2]['Days_Active']} días)")
print(f"      4. {stats_SR_Correlation.iloc[3]['Ticker']:<10} ({stats_SR_Correlation.iloc[3]['Days_Active']} días)")
print(f"      5. {stats_SR_Correlation.iloc[4]['Ticker']:<10} ({stats_SR_Correlation.iloc[4]['Days_Active']} días)")
...
# 2. SHARPE CORRELATION
print("\n   ► Sharpe Correlation:")
print(f"      1. {stats_SR_Correlation.iloc[0]['Ticker']:<10} ({stats_SR_Correlation.iloc[0]['Survival_Rate']:.2%})")
print(f"      2. {stats_SR_Correlation.iloc[1]['Ticker']:<10} ({stats_SR_Correlation.iloc[1]['Survival_Rate']:.2%})")
print(f"      3. {stats_SR_Correlation.iloc[2]['Ticker']:<10} ({stats_SR_Correlation.iloc[2]['Survival_Rate']:.2%})")
print(f"      4. {stats_SR_Correlation.iloc[3]['Ticker']:<10} ({stats_SR_Correlation.iloc[3]['Survival_Rate']:.2%})")
print(f"      5. {stats_SR_Correlation.iloc[4]['Ticker']:<10} ({stats_SR_Correlation.iloc[4]['Survival_Rate']:.2%})")
...
# 2. SHARPE CORRELATION
print("\n   ► Sharpe Correlation:")
print(f"      1. {stats_SR_Correlation.iloc[0]['Ticker']:<10} (Posición media: {stats_SR_Correlation.iloc[0]['Average_Position']:.2f}º)")
print(f"      2. {stats_SR_Correlation.iloc[1]['Ticker']:<10} (Posición media: {stats_SR_Correlation.iloc[1]['Average_Position']:.2f}º)")
print(f"      3. {stats_SR_Correlation.iloc[2]['Ticker']:<10} (Posición media: {stats_SR_Correlation.iloc[2]['Average_Position']:.2f}º)")
print(f"      4. {stats_SR_Correlation.iloc[3]['Ticker']:<10} (Posición media: {stats_SR_Correlation.iloc[3]['Average_Position']:.2f}º)")
print(f"      5. {stats_SR_Correlation.iloc[4]['Ticker']:<10} (Posición media: {stats_SR_Correlation.iloc[4]['Average_Position']:.2f}º)")
...
ranking_SR_Correlation = ranking_SR_Correlation_oos.map(id_to_ticker.get)
...
wealth_rolling_SR_Correlation_netos = pd.Series(transaction_costs(R_oos, ids_rolling_SR_Correlation, c=c), index=T)
...
plot_cumulative_returns({
    ...
    'SR Correlation': (wealth_rolling_SR_Correlation, wealth_buyhold_SR_Correlation, wealth_rolling_SR_Correlation_netos, 'purple'),
...
print(f"      Sharpe Corr  : €{wealth_buyhold_SR_Correlation.iloc[-1]:.4f}")
...
print(f"      Sharpe Corr  : €{wealth_rolling_SR_Correlation.iloc[-1]:.4f}")
...
print(f"      Sharpe Corr  : €{wealth_rolling_SR_Correlation_netos.iloc[-1]:.4f}")
...
variables = {
    ...
    'rankings_ids': {
        'SR': ids_rolling_SR,
        'SR_Correlation': ids_rolling_SR_Correlation,
        'MSR': ids_rolling_MSR,
    ...
    'rotation_mean': {
        ...
        'SR_Correlation': float(df_turnover_activos['Nuevos_SR_Correlation'].mean()),
...
serie_SR_Correlation_completa = series_completa['SR_Correlation']
...
with pd.ExcelWriter(OUTPUT_FILE, engine='openpyxl', mode='w') as writer:
    ...
    serie_SR_Correlation_oos.to_excel(writer, sheet_name='Serie Sharpe Corr OOS')
    ...
    serie_SR_Correlation_completa.to_excel(writer, sheet_name='Serie Sharpe Corr COMPLETA')
    ...
    ranking_SR_Correlation.to_excel(writer, sheet_name='Ranking Sharpe Corr OOS')
    ...
    stats_SR_Correlation.to_excel(writer, sheet_name='Survival_SR_Correlation', index=False)
